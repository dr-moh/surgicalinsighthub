...
async def extract_chunk(client, chunk_start, chunk_end, full_text, semaphore):
    async with semaphore:
        prompt = f"""
You are an expert data extractor. I have provided the entire text of a medical MCQ book: Schwartz's Principles of Surgery ABSITE and Board Review.

Your task is to extract Questions {chunk_start} to {chunk_end} ONLY.
For each question, extract the question text, the options, the correct answer, and the explanation (which is usually located immediately after the options).

Output a JSON array where each object has:
- "id": number (the question number {chunk_start} to {chunk_end})
- "question": string
- "options": object mapping "A", "B", "C", "D", "E" to the option strings
- "answer_key": string (e.g. "C")
- "raw_answer_text": string (the explanation paragraph that follows the answer)
...
async def main():
    print("=" * 65)
    print("  Stage 1: AI Chunk Extraction — Schwartz's 9th")
    print("=" * 65)

    client = genai.Client(vertexai=True, project="sih-mcq-pipeline", location="us-central1")
    
    print("Loading 1.3 MB book into memory...")
    with open(INPUT_FILE, "r", encoding="utf-8") as f:
        full_text = f.read()

    # Create chunks of 50 questions each
    chunks = []
    # Total questions is 1142.
    for i in range(1, 1150, 50):
        end = min(i + 49, 1142)
        chunks.append((i, end))

    print(f"Created {len(chunks)} chunks to process 1142 questions.")
